	...
	if (eventType.startsWith('charge.')) {
		...
		data = {
			object: {
				...
				currency: 'brl',
				customer: `cus_${faker.string.alphanumeric(14)}`,
				status: eventType === 'payment_intent.succeeded' ? 'succeeded' :
					eventType === 'payment_intent.payment_failed' ? 'requires_payment_method' :
		...
		data = {
			object: {
				...
				amount_paid: eventType === 'invoice.payment_succeeded' ? faker.number.int({ min: 2000, max: 80000 }) : 0,
				currency: 'brl',
				customer: `cus_${faker.string.alphanumeric(14)}`,
				status: eventType === 'invoice.payment_succeeded' ? 'paid' :
...
async function seed() {
	...
	// Estatísticas
	const eventCounts = webhookRecords.reduce((acc, record) => {
